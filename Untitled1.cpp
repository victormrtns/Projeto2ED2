...
//------------------------------------------------FUNÇÔES------------------------------------------
void Inserir(FILE *insere, indice* vet, int *tam_vet_carregar,FILE *prim);
...
int main(){
    ...
    if( (prim = fopen("primario.bin","r+b")) == NULL){

        prim = fopen("primario.bin","w+b");
...
// se header desatualizado --> recria arq primario.bin a partir do main -->
    ...
    do{
        ...
        switch(opcao){
            ...
            case 1:{
                Inserir(insere,vet, &tam_vet_carregar,prim);
                break;
            }
            case 2:{
